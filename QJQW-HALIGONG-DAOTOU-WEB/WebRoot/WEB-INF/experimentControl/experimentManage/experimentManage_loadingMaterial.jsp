...
<script type="text/javascript">
	...
	function opt1(NumberMaterial,deputy,id){
		var name = $("#span"+id).html();
		parent.window.opt(NumberMaterial,deputy,name);
	}
</script>
</head>
<body>	
...
<div class="content_box_table">
  <form action="experimentManage/jzExperimentMachine?deputy=3" method="post" name="Form" id="Form">
	    ...
	    <c:forEach items="${materialList}" var="var" varStatus="vs">
	      <div class="each_list_wrap">
			...
			<span class="list_title skz_hide skz_list0">材料名称：${var.NameMaterial }</span> 
 			...
 			<span class="skz_hide skz_list3">ISO加工分类：${var.ISOCuttingMaterial }</span> 
			...
			<span class="opera_wrap  skz_hide  skz_list6">
			  <span class="btn btn_xg" onclick="opt1('${var.NumberMaterial}',3,'${var.id_Material}');">选择</span>
			</span>					
			<div class="clear"></div>
			<!-- 展开详情   -->
					...
					<div class="box_xiangxi">
						<div class="box_u1">
								<ul class="ul1">
								  ...
								  <li><span>材料类别</span><i>
								  <c:forEach items="${categoryMaterialList}" var="var1" varStatus="vs">
				 						<c:if test="${var1.id_CategoryMaterial == var.id_CategoryMaterial}"><div id="span${var.id_Material }" >${var1.name_CategoryMaterial }</div></c:if>
									</c:forEach>
								  </i></li>
  ...
  <div class="page-header position-relative" style="vertical-align:top;padding-top:10px;padding-left:20px;">
	<div class="pagination">
	  <ff:page mhFrom="Form" showReSize="true"  field="page" onlyOneShow="true" showListNo="false" action="experimentManage/jzExperimentMachine?deputy=3"/>
	</div>
  </div>
</div>
