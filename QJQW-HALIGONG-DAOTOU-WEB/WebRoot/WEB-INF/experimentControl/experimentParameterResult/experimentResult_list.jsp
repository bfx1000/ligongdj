...
<head>
	...
	<script type="text/javascript">
		 ...
		 function insertResult(id){
			 var html = '<iframe class="result_iframe" id="iframe" frameborder="0" src="/experimentParameterResult/CheckExperimentResultList?id_ExperimentParameterResult='+id+'&&WhetherVariable=1" style="width:100%;height:100%;"></iframe>>';
...
<form action = "/experimentParameterResult/goExperimentResult?zT=1" name="userForm" id="userForm" method="post">
  <ul class="box_text">
	 ...
	 <li>
	  ...
	  <div id = "div">
	  	...
	  	<li>
	  		<span id="lititle">实验机床&nbsp;：&nbsp;</span>${ExperimentManage.ExperimentMachine}
	  	</li>
	  	<li>
	  		<span id="lititle">试验件&nbsp;：&nbsp;</span>${ExperimentManage.ExperimentPart}
	  	</li>
	  	<li>
	  		<span id="lititle">材料牌号&nbsp;：&nbsp;</span>${ExperimentManage.NumberMaterialExperimentPart}
	  	</li>
	  	<li>
	  		<span id="lititle">材料种类&nbsp;：&nbsp;</span>${ExperimentManage.CategoryMaterialExperimentPart}
	  	</li>
	  	<li>
	  		<span id="lititle">试验刀具&nbsp;：&nbsp;</span>${ExperimentManage.ExperimentTool}
	  	</li>
	  	<li>
	  		<span id="lititle">刀具材料&nbsp;：&nbsp;</span>${ExperimentManage.ExperimentToolMaterial}
	  	</li>
	  	<li>
	  		<span id="lititle">切削液&nbsp;：&nbsp;</span>${ExperimentManage.ExperimentCuttingFluid}
	  	</li>
	  	<li>
	  		<span id="lititle">加工类别&nbsp;：&nbsp;</span>${ExperimentManage.ExperimentMachiningType}
	  	</li>
	  </div>
  </ul>
  </form>
						   ...
						   <%}%>
							...
							<%for(int i =0 ;i<list.size();i++){%>
								...
								<%for(int j =0;j<list1.size();j++){%>
									<td id="col_row<%=i+1%>_<%=j%>" ><%=list.get(i).get(list1.get(j))%></td>
								<%}%>
								<th id="col_row<%=i+1%>"><a class="btn btn_bg_orange b_block" onclick="insertResult(<%=list.get(i).get("id_ExperimentParameterResult")%>);">查看结果</a></th>
								</tr>
							<%}%>
					</c:when>
						<c:otherwise>
