...
<script type="text/javascript">
	...
	function onSearchKey(){
		var url = "<%=path%>/machineNodeInform/queryMachineNodeInformList";
...
<script type="text/javascript">
	...
	//新增
	function add(){
		window.location.href='<%=path%>/machineNodeInform/goAddMachineNodeInform?id=';
	}
	
	//修改
	function edit(id){
		window.location.href='<%=path%>/machineNodeInform/goAddMachineNodeInform?id=' + id;
	}
	...
	//删除
	function del(id_MachineNodeInform){
		if(confirm("确定要删除吗?")){
			var url = "<%=path%>/machineNodeInform/deleteMachineNodeInform?id_MachineNodeInform=" + id_MachineNodeInform;
			document.Form.action = url;
...
<div class="content_box_table">
  <form action="machineNodeInform/queryMachineNodeInformList" method="post" name="Form" id="Form">
	    ...
	    <c:forEach items="${machineNodeInformList}" var="var" varStatus="vs">
	      <div class="each_list_wrap">
			...
			<span class="opera_wrap  skz_hide  skz_list6">
			  <span class="btn btn_xg" onclick="edit('${var.id_MachineNodeInform}');">修改</span>
			  <span class="btn btn_sc" onclick="del('${var.id_MachineNodeInform}');">删除</span>
			</span>					
			<div class="clear"></div>
			<!-- 展开详情   -->
			<div id="ulli_${var.id_MachineNodeInform}" class="ulli" style="display: none;">
			  <div class="box_xiangxi">
				...
				<div class="box_p">
				  <h4>详细信息</h4>
				  <p>
				    <c:forEach items="${var.informDetailMachineNodeInformList}" var="var1" varStatus="vs">
				      <span name='informDetailTurningHolders_img' >${var1.name_InformDetailMachineNodeInform }</span>
				      <a class="btn" onclick="onShowPdf('${var1.url_InformDetailMachineNodeInform }');">预览</a>
				      <br/>
				    </c:forEach>
				  </p>
				</div>
  ...
  <div class="page-header position-relative" style="vertical-align:top;padding-top:10px;padding-left:20px;">
	<div class="pagination">
	  <ff:page mhFrom="Form" showReSize="true"  field="page" onlyOneShow="true" showListNo="false" action="machineNodeInform/queryMachineNodeInformList"/>
	</div>
  </div>
</div>
