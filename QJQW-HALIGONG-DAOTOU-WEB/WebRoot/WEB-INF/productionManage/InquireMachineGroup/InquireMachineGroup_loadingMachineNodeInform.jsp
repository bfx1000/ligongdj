...
<script type="text/javascript">
	...
	function onSearchKey(){
		var url = "<%=path%>/InquireMachineGroup/loading?deputy=1";
	...
	function opt1(NumberMachineNode,NumberMachine,TypeMachineNode,DepartmentMachineNode,WorkshopMachineNode,PositionMachineNode){
		var name = $("#span_"+TypeMachineNode).text();
		window.parent.opt(1,NumberMachineNode,NumberMachine,name,DepartmentMachineNode,WorkshopMachineNode,PositionMachineNode);
	}
</script>
...
<div class="content_box_table">
  <form action="InquireMachineGroup/loading?deputy=1" method="post" name="Form" id="Form">
	    ...
	    <c:forEach items="${machineNodeInformList}" var="var" varStatus="vs">
	      <div class="each_list_wrap">
			...
			<span class="opera_wrap  skz_hide  skz_list6">
			  <span class="btn btn_xg" onclick="opt1('${var.NumberMachineNode}','${var.NumberMachine}','${var.TypeMachineNode }','${var.DepartmentMachineNode }','${var.WorkshopMachineNode }','${var.PositionMachineNode }');">选择</span>
			</span>					
			<div class="clear"></div>
			<!-- 展开详情   -->
				...
				<div class="box_u1">
				  <ul class="ul1">
					...
					<li><span>机床类别</span>
						<i>
							<c:forEach items="${categoryMachineNodeInform}" var="var1" varStatus="vs">
	 							<c:if test="${var1.id_CategoryMachineNodeInform == var.TypeMachineNode}"><span id="span_${var.TypeMachineNode }">${var1.name_CategoryMachineNodeInform }</span></c:if>
				 			</c:forEach>
 						</i>
 					</li>
  ...
  <div class="page-header position-relative" style="vertical-align:top;padding-top:10px;padding-left:20px;">
	<div class="pagination">
	  <ff:page mhFrom="Form" showReSize="true"  field="page" onlyOneShow="true" showListNo="false" action="InquireMachineGroup/loading?deputy=1"/>
	</div>
  </div>
</div>
