...
<head>
	...
	<script type="text/javascript">
		...
		function onSearchKey(){
			var IDDeviceManufPlan = $("#IDDeviceManufPlan_loading").val();
			var url = "<%=path%>/InquireMachineGroup/loading?deputy=3&IDDeviceManufPlan="+IDDeviceManufPlan;
...
<div class="content_box_table">
	<form action="InquireMachineGroup/loading?deputy=3" method="post" name="Form" id="Form">
	...
	<input id="IDDeviceManufPlan_loading" type="hidden" value="${pd.IDDeviceManufPlan }" />
	  			...
	  			<c:forEach items="${ManufacturePlan}" var="var" varStatus="vs">	
					<div class="each_list_wrap"  >
						...
						<span class="opera_wrap  skz_hide  skz_list5">
							<span class="btn btn_xg" onclick="window.parent.opt(3,'${var.PartNoManufPlan}','${var.NameManufPlan}','${var.DateStartManufPlan}','${var.DateFinishManufPlan}','${var.id_ManufacturePlan }');">选择</span>
						</span>
						<div class="clear"></div>	
...
<!-- 		  <a class="btn" onclick="makeAll('确定要删除选中的数据吗?');" title="批量删除" >批量删除</a> -->
		...
		<div class="pagination">
	  		<ff:page mhFrom="Form" showReSize="true"  field="page" onlyOneShow="true" showListNo="false" action="InquireMachineGroup/loading?deputy=3&IDDeviceManufPlan=${pd.IDDeviceManufPlan }"/>
		</div>
	</div>
</div>
</body>
