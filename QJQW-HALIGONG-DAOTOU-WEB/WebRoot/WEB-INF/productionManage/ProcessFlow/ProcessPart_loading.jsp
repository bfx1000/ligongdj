...
<head>
	...
	<script type="text/javascript">
		...
		function onSearchKey(gxId){
			var url = "<%=path%>/ProcessFlow/jzAll?deputy=3&gxId="+gxId;
	...
	<script type="text/javascript">
		...
		function opt1(id,gxId,NameProcessPart,NumberProcessPart,TypeProcessPart,NumberMaterialProcessPart,CategoryMaterialProcessPart,AssemblyProcessPart,NoAssemblyProcessPart){
			var name = $("#span"+id).html();
			window.parent.opt(3,gxId,NumberProcessPart,name,NameProcessPart,NumberMaterialProcessPart,CategoryMaterialProcessPart,AssemblyProcessPart,NoAssemblyProcessPart);
		}
...
<div class="content_box_table">
	<form action="ProcessFlow/jzAll?deputy=3" method="post" name="Form" id="Form">
	  			...
	  			<c:forEach items="${ProcessPart}" var="var" varStatus="vs">	
					<div class="each_list_wrap"  >
						...
						<span class="opera_wrap  skz_hide  skz_list5">
							&nbsp;&nbsp;<span class="btn btn_xg" onclick="opt1(${var.id_ProcessPart },'${gxId }','${var.NumberProcessPart}','${var.NameProcessPart}','${var.TypeProcessPart}','${var.NumberMaterialProcessPart}','${var.CategoryMaterialProcessPart}','${var.AssemblyProcessPart}','${var.NoAssemblyProcessPart}');">选择</span>
						</span>
						<div class="clear"></div>	
							...
							<!--<img src="images/dtcp_img1.png" class="img1" alt="哈理工刀头" title="哈理工刀头"/> -->
								<div class="box_u1">
									<ul class="ul1">
								  		...
								  		<li><span>零件类型</span><i>
								  			<c:forEach items="${CategoryTypeProcessPart}" var="var1" varStatus="vs">
													<c:if test="${var1.id_CategoryTypeProcessPart == var.TypeProcessPart}"><div id="span${var.id_ProcessPart }" >${var1.name_CategoryTypeProcessPart }</div></c:if>	
											</c:forEach>
										</i></li>
...
<!-- 		  <a class="btn" onclick="makeAll('确定要删除选中的数据吗?');" title="批量删除" >批量删除</a> -->
		...
		<div class="pagination">
	  		<ff:page mhFrom="Form" showReSize="true"  field="page" onlyOneShow="true" showListNo="false" action="ProcessFlow/jzAll?deputy=3&gxId=${gxId }"/>
		</div>
	</div>
</div>
</body>
