...
<script type="text/javascript">
	...
	function onSearchKey(){
		var url = "<%=path%>/StaffManuf/StaffManufList";
...
<script type="text/javascript">
	...
	//新增
	function add(){
		window.location.href='<%=path%>/StaffManuf/goAddStaffManuf?id=';
	}
	
	//修改
	function edit(id){
		window.location.href='<%=path%>/StaffManuf/goAddStaffManuf?id=' + id;
	}
	...
	function del(id_ManufactureResult){
		if(confirm("确定要删除吗?")){
			var url = "<%=path%>/StaffManuf/deleteStaffManufList?id_StaffManuf=" + id_ManufactureResult;
			document.Form.action = url;
...
<div class="content_box_table">
  <form action="StaffManuf/StaffManufList" method="post" name="Form" id="Form">
	    ...
	    <c:forEach items="${StaffManuf}" var="var" varStatus="vs">
	      <div class="each_list_wrap">
			...
			<span class="opera_wrap  skz_hide  skz_list4">
			  <span class="btn btn_xg" onclick="edit('${var.id_StaffManuf}');">修改</span>
			  <span class="btn btn_sc" onclick="del('${var.id_StaffManuf}');">删除</span>
			</span>					
			<div class="clear"></div>
			<!-- 展开详情   -->
  ...
  <div class="page-header position-relative" style="vertical-align:top;padding-top:10px;padding-left:20px;">
	<div class="pagination">
	  <ff:page mhFrom="Form" showReSize="true"  field="page" onlyOneShow="true" showListNo="false" action="StaffManuf/StaffManufList"/>
	</div>
  </div>
</div>
