...
<div class="navbar navbar-inverse">
		  <div class="navbar-inner">
		   <div class="container-fluid">
 			  <a class="brand" href="">海伦市保障房系统</a>
			  
			  <ul class="nav ace-nav pull-right">
					<li class="light-blue user-profile">
						<a class="user-menu dropdown-toggle" href="javascript:alert('预留功能,待开发');" data-toggle="dropdown">
							<%-- <img alt="${sessionScope.USER_NAME }" src="<%=path%>/resources/images/T1RppXXh4eXXXXXXXX.jpg" class="nav-user-photo" /> --%>
							<span id="user_info">
...
// 					hf(b);
// 					var url = '/qjqwmain/systemuser/head/setSKIN?SKIN='+b+'&tm='+new Date().getTime();
...
// 			})
			...
			function getUname(){
				$.ajax({
					type: "POST",
					url: '/qjqwmain/systemuser/head/getUname?tm='+new Date().getTime(),
			    	data: encodeURI(""),
					dataType:'json',
					...
					success: function(data){
						//alert(data.list.length);
						 $("#user_info").html('<small>Welcome</small> '+data.list.name+'');
						 
						 
						 USER_ID = data.list.user_id;//用户ID
						 hf(data.list.SKIN)//皮肤
						 
						 if(data.list.user_name != 'admin'){
			...
			function editUserH(){
				 ...
				 diag.Drag=true;
				 diag.Title ="个人资料";
				 diag.URL = '/qjqwmain/systemuser/goEditU?USER_ID='+USER_ID+'&fx=head';
				 diag.Width = 225;
				 diag.Height = 389;
			...
			function editSys(){
				 ...
				 diag.Drag=true;
				 diag.Title ="系统设置";
				 diag.URL = '/qjqwmain/head/goSystem';
				 diag.Width = 600;
				 diag.Height = 596;
				 diag.CancelEvent = function(){ //关闭事件
					diag.close();
			...
			function productCode(){
				 ...
				 diag.Drag=true;
				 diag.Title ="代码生成器";
				 diag.URL = '/qjqwmain/head/goProductCode';
				 diag.Width = 800;
				 diag.Height = 450;
				 diag.CancelEvent = function(){ //关闭事件
					hangge();
			...
			function zidian(){
				 ...
				 diag.Drag=true;
				 diag.Title ="数据字典";
				 diag.URL = '/qjqwmain/zidian?PARENT_ID=0';
				 diag.Width = 799;
				 diag.Height = 460;
				 diag.CancelEvent = function(){ //关闭事件
					diag.close();
			...
			//切换菜单
			function changeMenu(){
				window.location.href='/qjqwmain/index?changeMenu=yes';
			}
