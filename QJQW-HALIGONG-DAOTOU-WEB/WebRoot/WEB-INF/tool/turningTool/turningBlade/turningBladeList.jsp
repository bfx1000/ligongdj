...
<script type="text/javascript">
	...
	function onSearchKey(){
		var url = "<%=path%>/turningBlade/queryTurningBladeList";
...
<script type="text/javascript">
	...
	//新增
	function add(){
		window.location.href='<%=path%>/turningBlade/goAddTurningBlade?id=';
	}
	
	//修改
	function edit(id){
		window.location.href='<%=path%>/turningBlade/goAddTurningBlade?id=' + id;
	}
	...
	function del(id_TurningBlade){
		if(confirm("确定要删除吗?")){
			var url = "<%=path%>/turningBlade/deleteTurningBlade?id_TurningBlade=" + id_TurningBlade;
			document.Form.action = url;
...
<div class="content_box_table">
  <form action="turningBlade/queryTurningBladeList" method="post" name="Form" id="Form">
	    ...
	    <c:forEach items="${turningBladeList}" var="var" varStatus="vs">
	      <div class="each_list_wrap">
			...
			<span class="opera_wrap  skz_hide  skz_list9">
			  <span class="btn btn_xg" onclick="edit('${var.id_TurningBlade}');">修改</span>
			  <span class="btn btn_sc" onclick="del('${var.id_TurningBlade}');">删除</span>
			</span>					
			<div class="clear"></div>
			<!-- 展开详情   -->
			<div id="ulli_${var.id_TurningBlade}" class="ulli" style="display: none;">
			  ...
			  <div class="box_p">
			    <h4>刀具图片</h4>
			    <p>
			      <c:forEach items="${var.pictureBladeTurningList}" var="var1" varStatus="vs">
			        <img name='pictureTurningHolders_img' width='100px' height='100px' src='<%=path%>/goods${var1.url_PictureBladeTurning }' />
			        <a class="btn" onclick="onShowPdf('${var1.url_PictureBladeTurning }');">预览</a>
			        <br/>
			      </c:forEach>
			    </p>
			  </div>
			  <div class="box_p">
			    <h4>详细图片</h4>
			    <p>
			      <c:forEach items="${var.pictureDetailBladeTurningList}" var="var1" varStatus="vs">
			        <img name='pictureTurningHolders_img' width='100px' height='100px' src='<%=path%>/goods${var1.url_PictureDetailBladeTurning }' />
			        <a class="btn" onclick="onShowPdf('${var1.url_PictureDetailBladeTurning }');">预览</a>
			        <br/>
			      </c:forEach>
			    </p>
			  </div>
				...
				<div class="box_p">
				  <h4>详细信息</h4>
				  <p>
				    <c:forEach items="${var.informDetailBladeTurningList}" var="var1" varStatus="vs">
				      <span name='informDetailTurningHolders_img' >${var1.name_InformDetailBladeTurning }</span>
				      <a class="btn" onclick="onShowPdf('${var1.url_InformDetailBladeTurning }');">预览</a>
				      <br/>
				    </c:forEach>
				  </p>
				</div>
				<div class="box_p">
				  <h4>应用实例</h4>
				  <p>
				    <c:forEach items="${var.applicationBladeTurningList}" var="var1" varStatus="vs">
				      <span name='informDetailTurningHolders_img' >${var1.name_ApplicationBladeTurning }</span>
				      <a class="btn" onclick="onShowPdf('${var1.url_ApplicationBladeTurning }');">预览</a>
				      <br/>
				    </c:forEach>
				  </p>
				</div>
  ...
  <div class="page-header position-relative" style="vertical-align:top;padding-top:10px;padding-left:20px;">
	<div class="pagination">
	  <ff:page mhFrom="Form" showReSize="true"  field="page" onlyOneShow="true" showListNo="false" action="turningBlade/queryTurningBladeList"/>
	</div>
  </div>
</div>
