...
//						"' title='' class='", view.makeNodeLineClass(setting, node), "' treeNode", consts.id.SWITCH,"></button>");
					...
					html.push("<button type='button' hidefocus='true' id='", node.tId, consts.id.ICON,
						"' title='' treeNode", consts.id.ICON," class='", view.makeNodeIcoClass(setting, node), "' style='", view.makeNodeIcoStyle(setting, node), "'></button><a id='", node.tId, consts.id.A, "' class='level", node.level,"' treeNode", consts.id.A," onclick=\"", (node.click || ''),
					...
					var tab_str = '';
					for(var k=0;k<node.level;k++){
						tab_str = tab_str + "&nbsp&nbsp&nbsp&nbsp";
					}
					var name = setting.view.nameIsHTML ? node[nameKey] : node[nameKey].replace(/&/g,'&amp;').replace(/</g,'&lt;').replace(/>/g,'&gt;');
					html.push("<span class='span_add_left' id='", node.tId, consts.id.SPAN,
						"'>",tab_str,name,"</span>");
					data.getInnerAfterA(setting, node, html);
					if(node.isfile == 0 ){//是文件夹
						html.push("</a><a class='caozuo_btn'><span class='span_add' onclick='onAdd("+node.id+")'>添加</span></a>");
					}else{//是文件 
						html.push("</a><a class='caozuo_btn'><span class='span_add' onclick='onEdit("+node.id+")'>修改</span><span class='span_add' onclick='onDel("+node.id+")'>删除</span></a>");
					}
