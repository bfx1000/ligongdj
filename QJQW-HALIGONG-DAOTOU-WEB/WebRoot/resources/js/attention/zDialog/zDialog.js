 ...
 * 最后修正：2010-6-29
 **/
var IMAGESPATH = '/resources/js/attention/zDialog/skins/blue/'; //图片路径配置，如在有iframe情况下使用，此路径为最顶层窗口页面的相对路径
    ...
    if (this.Drag && topWin.Drag){//注册拖拽方法
		...
		dragBody.onDragEnd = function(left,top,mouseX,mouseY){
			...
			if(ielt7){
				...
				if(top+39>bd.scrollTop+bd.clientHeight)
					this.style.top=bd.scrollTop+bd.clientHeight-39+'px';
			}else{
				if(top<0)
					this.style.top='0px';
				if(top+39>bd.clientHeight)
					this.style.top=bd.clientHeight-39+'px';
			}
		}
	}
};
...
Dialog.prototype.create = function () {
    ...
    var DialogDivHeight = this.Height + 39 + 13 + (this.ShowButtonRow ? 40 : 0) + (this.ShowMessageRow ? 50 : 0);
  ...
  <table id="_DialogTable_{thisID}" width="' + (this.Width + 26) + '" cellspacing="0" cellpadding="0" border="0" onselectstart="return false;" style="-moz-user-select: -moz-none; font-size:12px; line-height:1.4;border-collapse: collapse;">\
    <tr id="_Draghandle_{thisID}" style="' + (this.Drag ? "cursor: move;" : "") + '">\
      <td width="13" height="39" style="><div style="width: 13px;"></div></td>\
      <td height="39" style="background-color: #069dd5;><div style="padding: 9px 0 0 4px; float: left; font-weight: bold; color:#fff;"><span id="_Title_{thisID}" style=\"margin:0 auto; color: #FFF; font:bold 18px \'Microsoft YaHei\';display:block;line-height:39px;width:100%;text-align:center;\">' + this.Title + '</span></div>\
        <div id="_ButtonClose_{thisID}" onclick="Dialog.getInstance(\'{thisID}\').cancelButton.onclick.apply(Dialog.getInstance(\'{thisID}\').cancelButton,[]);" onmouseout="this.style.backgroundImage=\'url({IMAGESPATH}dialog_closebtn.gif)\'" onmouseover="this.style.backgroundImage=\'url({IMAGESPATH}dialog_closebtn_over.gif)\'" style=" position: absolute;top:15px; cursor: pointer; right:15px; height: 17px; width: 28px; background: url({IMAGESPATH}dialog_closebtn.gif) 0 0;' + (ielt7 ? "margin-top: 3px;" : "") + (this.ShowCloseButton ? "" : "display:none;") + '"></div></td>\
      <td width="13" height="39" style=""><div style="width: 13px;"><a id="_forTab_{thisID}" href="#;"></a></div></td>\
    </tr>\
    <tr valign="top">\
    ...
    <tr>\
      ...
      <td style=""></td>\
      <td width="13" height="13" style=""><a onfocus=\'getDom("_forTab_{thisID}").focus();\' href="#;"></a></td>\
    </tr>\
  </table>\
</div>\
';
...
Dialog.prototype.close = function () {
    ...
    if (topWin.Dialog._dialogArray.length > 0) {
        ...
        Dialog.bgDiv.style.zIndex = "999999999999";
        Dialog.bgDiv.style.display = "none";
        if(HideScrollbar){
	        var topWinBody = topDoc.getElementsByTagName(topDoc.compatMode == "BackCompat" ? "BODY" : "HTML")[0];
...
Dialog.getBgdiv = function () {
    ...
    if (!bgdiv) {
        ...
        bgdiv.style.cssText = "position:absolute;left:0px;top:0px;width:100%;height:100%;z-index:999999999998";
