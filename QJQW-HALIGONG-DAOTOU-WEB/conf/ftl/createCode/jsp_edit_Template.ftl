...
<body>
	<form action="/qjqwmain/${objectNameLower}/${r"${msg }"}" name="Form" id="Form" method="post">
		<input type="hidden" name="${objectNameLower}_id" id="${objectNameLower}_id" value="${r"${"}${objectNameLower}${r"."}${objectNameLower}_id${r"}"}"/>
		<div id="zhongxin">
...
<#list fieldList as var>
	<#if var[3] == "是">
		<#if var[1] == 'Date'>
			<tr>
				<td>${var[2] }:</td><td><input class="span10 date-picker" name="${var[0] }" id="${var[0] }" value="${r"${"}${objectNameLower}${r"."}${var[0] }${r"}"}" type="text" data-date-format="yyyy-mm-dd" readonly="readonly" placeholder="${var[2] }" title="${var[2] }"/></td>
			</tr>
		<#elseif var[1] == 'Integer'>
			<tr>
				<td>${var[2] }:</td><td><input type="number" name="${var[0] }" id="${var[0] }" value="${r"${"}${objectNameLower}${r"."}${var[0] }${r"}"}" maxlength="32" placeholder="这里输入${var[2] }" title="${var[2] }"/></td>
			</tr>
		<#else>
			<tr>
				<td>${var[2] }:</td><td><input type="text" name="${var[0] }" id="${var[0] }" value="${r"${"}${objectNameLower}${r"."}${var[0] }${r"}"}" maxlength="32" placeholder="这里输入${var[2] }" title="${var[2] }"/></td>
			</tr>
		</#if>
	</#if>
</#list>
